...
def cut_envelope(waveform,omega,evlon,evlat,stlon,stlat,slowness_s,slowness_r,in_env):
        ...
        for x in range(0,len(waveform)):
                ...
                dtcs_total = dtcs_s + dtcs_r
                #max_pos = np.argmax(envelope_amp[max_env_pos +int(sample*dtcs_total) - int(sample*float(period)*5):max_env_pos +int(sample*dtcs_total) + int(sample*float(period)*5)])
                #max_pos = np.argmax(envelope_amp[max_env_pos +int(sample*dtcs_total) - int(sample*float(period)*2):max_env_pos +int(sample*dtcs_total) + int(sample*float(period)*2)])
                max_pos = np.argmax(envelope_amp[max_env_pos - int(sample*float(period)*1.5):max_env_pos + int(sample*float(period)*1.5)])
#                print(max_env_pos - int(sample*float(period)*2), max_env_pos + int(sample*float(period)*2), max_pos)
#                sys.exit()
#                final_max = max_env_pos + max_pos + int(sample*dtcs_total)- int(sample*float(period)*5)
                final_max = max_env_pos+max_pos-int(sample*float(period)*1.5)
#                print(final_max, max_env_pos+int(sample*dtcs_total))
                ...
                plt.plot(waveform[x,:])
                plt.plot(envelope_amp,'r-')
                #plt.plot([max_env_pos+int(sample*dtcs_total),max_env_pos+int(sample*dtcs_total)],[-1,1],'b*')
                plt.plot([max_env_pos,max_env_pos],[-1,1],'b*')
                plt.plot([final_max,final_max],[-1,1],'r*')
                plt.plot([right,right],[-1,1],'k-')
                plt.plot([left,left],[-1,1],'k-')
                plt.plot(waveform[x,:]-new_waveform[:])
                plt.xlim(1500,1900)
                plt.show()
#                sys.exit()
#                '''
...
def cal_snr(data,dist,cut_left,cut_right):
        ...
        for i in range(0,len(data)):
                ...
                #snr_fol = np.max(np.abs(data[i,center+left:]))/np.sqrt(np.mean(data[i,cut_right[i]:]**2))
                #avg_sqrt = np.sqrt(np.mean(data[i,noi_b:]**2)+np.mean(data[i,center_pts:sig_b]**2))
                snr_fol = np.max(np.abs(data[i,center+left:]))/np.sqrt(np.mean(data[i,noi_b:]**2))
                #snr_fol = np.max(np.abs(data[i,center+left:]))/avg_sqrt
                #print(sig_b,noi_b,snr_fol)
...
if __name__ == "__main__":
        ...
        if len(sys.argv[:]) != 6:
                print('proper usage: python ~/bin/bfpw/numba_yadong.py EQ Sta_Center Period Slowness_Step')
                print('Ex: python ~/bin/bfpw/numba_yadong.py M01 M16 2 0.1 0.01')
                sys.exit()
        tStart = time.time()
        n = 0
...
#        dist_stack = geodistance(min(evlon), min(evlat), max(stlon), max(stlat))
#        print(dist_stack)
#        sys.exit()
        ...
        dist = np.array(dist)
        #ref_period=round(float(period)+1.0,2)
        ref_period=round(float(period)+0.5,2)
#        ref_period=round(float(period)+10.0,2)
        if str(ref_period).split('.')[-1] == '0':
                ref_period=int(ref_period)
    
        ref_file = 'Output/period_'+str(ref_period)+'_fund_final.txt'
#        if os.path.isfile('velocity/fund/vel_'+str(ref_period)+'.txt'):
        if os.path.isfile(ref_file): #period_6_fund_final.txt
...
#                        print(coarse_start_s, coarse_start_r)
...
#        print(coarse_slowness_s, coarse_slowness_r)
#        sys.exit()
...
# Second Grid Search=========================================================================================================================================================================
        ...
        fmt='%1.4f', '%1.4f', '%1.5f'
        '''
        len_criteria = float(period)/10
        if len_criteria > 0.4:
                len_criteria = 0.4
        print(np.mean(snr_fol), len(data), len(re_data_2),abs(len(snr_fol) - len(re_data_2)),len_criteria*len(data), math.ceil(len_criteria*len(data)))
        sys.exit()
#        '''
        len_criteria = 0.4
        #if np.mean(snr_fol) < 5.0 or abs(len(snr_fol) - len(re_data_2)) >= int(len_criteria*len(data)) or float(period)>3 or float(period)<2:
        #if np.mean(snr_fol) < 8.0 or abs(len(snr_fol) - len(re_data_2)) >= int(len_criteria*len(data)) or float(period)>=4 or float(period)<=1:
        #print(np.mean(snr_fol), np.mean(amp_data_2), len(data), len(re_data_2),abs(len(snr_fol) - len(re_data_2)),len_criteria*len(data), math.ceil(len_criteria*len(data)))
        #sys.exit()
